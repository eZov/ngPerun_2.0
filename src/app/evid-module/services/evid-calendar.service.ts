...
export class EvidCalendarService {
    ...
    getCalendarData(_empid: number, _MM: number, _YYYY: number): Observable<boolean> {
        let _sptype: string = "eviddnevnik";
    ...
    saveCalendarData(_superlock?: boolean): Observable<boolean> {
        ...
        return this.httpCoreService.postData<EvidDnevnik[]>(`${this.httpCoreService.baseUrl}${_sptype}`, this.evidDnevnikData).pipe(
            map((value: EvidDnevnik[]) => {
                this._evidDnevnikData = value;
                this.resetSati();
    ...
    processCalendarData() {
        ...
        // anuliraj sate prema šifri  (bez ovoga se uduplavaju sati nakon refresh)
    ...
    selEvidSifra(args: any) {
        this._selEvid = new EvidCalendar();

        for (let el of this.sfrevidprisData) {
            el.itemType = el.itemType !== undefined ? Math.abs(el.itemType) : 0;

            if (el.itemType == args) {
                el.itemType = -el.itemType;

                this._selEvid = new EvidCalendar();
                this._selEvid.sifra = el.sifra;
                this._selEvid.css = "sfr-" + el.sifra?.toLowerCase()
            }
        }
    ...
    autoDelete() {
        ...
        if (this._selEvid != undefined) {
            ...
            this.evidCalendarData.forEach(el => {
                ...
                console.log("==> (autoDelete) 1: " + JSON.stringify(el));

                /*
                Filtriranje evidDnevnika prema datumu i keyday
                */
                let _evidDnevnik: EvidDnevnik;
                _evidDnevnik = this._evidDnevnikData.filter(eld => {
                    let _elDay = new Date(eld.datum || '');
                ...
                console.log("==> (autoDelete) 2: " + JSON.stringify(_evidDnevnik));

                //HACK: Dan je locked, nadredjena evidencija - EXIT
                if (_evidDnevnik.locked && this.usersession.user.role === "uposlenik") return el;

                // Šifre se podudaraju sa odabranom šifrom
                if ((el.sifra === this._selEvid.sifra) ||
                    ((el.sifra || '').indexOf('GO') >= 0 && this._selEvid.sifra === "GO")) {
                ...
                /*
                HACK: Koja je prava svrha ovoga?
                */
                if (el.sifra != "RRD") {
    ...
    getGoStatus(_empID: number, _YYYY: number): Observable<boolean> {
    ...
    evidDnevnikSend(
        ...
        _MM: number,
        _YYYY: number,
        _value: boolean = false): Observable<boolean> {
        ...
        return this.httpCoreService.getData<EvidDnevnik[]>(`${this.httpCoreService.baseUrl}${_sptype}?vrsta=${_valStr}&EmployeeID=${_empID}&mm=${_MM}&yyyy=${_YYYY}`).pipe(
            map((value: EvidDnevnik[]) => {
                this._evidDnevnikData = value;
                let _dummyDataGrid: string = JSON.stringify(this._evidDnevnikData);
                this.convertToDay(_dummyDataGrid);
                this.processCalendarData();
                return true;
            }),
